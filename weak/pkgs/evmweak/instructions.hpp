...
inline void calldataload(StackTop stack, ExecutionState& state) noexcept
{
    ...
    cout << "🐾[EVMWEAK]  calldataload called, stack top is " << static_cast<int>(index) << '\n';
...
{
  cout << "🐾[EVMWEAK]  calldatasize called, the size is " << state.msg->input_size << '\n';
