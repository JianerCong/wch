...
#include <evmweak/instructions_traits.hpp>
...
using enum evmweak::Opcode;
using evmweak::Opcode;

// TODO: Pull bytecode in evmweak namespace
...
inline std::string decode(bytes_view bytecode)
{
    ...
    for (auto it = bytecode.begin(); it != bytecode.end(); ++it)
    {
        ...
        if (const auto name = evmweak::instr::traits[opcode].name; name)
        {
